...
public class BinFunction {
	...
	public Set computeOff() throws Exception {
		if (true)
			throw new Exception("forget the off-set");
		Set onDc = new Set(); // onDc contains all implicants that are not in the offset. So not onDC is the offset, which has to be expanded to get a disjunctive form
		onDc.addAll(this.on);
		onDc.addAll(this.dc);
