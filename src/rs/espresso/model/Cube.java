...
/**
 * The class Cube stores one and-Combination of variable dependencies of a Function. This class
 * is meant to be extended in other units to store additional info about the cube.
 * 
...
public class Cube {
	...
	/**
	 * Copies the current cube and can also be used, to convert it up to an extending class. This
	 * class can override clone() to add copies of the additional information.
	 * 
	 * @param c
	 *           A class, that extends Cube can be given here. This class must have a constructor
	 *           like Cube (int width)
	 * @return A new cube of class cubeType, that contains all Cube information of the current
	 *         cube (this)
	 */
	...
	public boolean equals(Cube foreign) {
		if (width != foreign.width)
			return false;
		for (int i = 0; i < cube.length; i++)
			if (cube[i] != foreign.cube[i])
				return false;
		return true;
	}

	/**
	 * @return Returns the (cardinality ld 2) of the set specified by this cube so that 2^result =
	 *         cardinality; result = -1, if cardinality is 0 (cube contains invalid)
	 */
	public int cardinality2() {
		...
		for (int i = 0; i < width / 32; i++) {
			long p = cube[i];
			for (int j = 0; j < 32; j++) {
				if ((p & BinFunction.DC) == BinFunction.INV)
					return -1;
				if ((p & BinFunction.DC) == BinFunction.DC)
					r++;
				p = p >>> 2;
			}
		}
	...
	public boolean isValid() {
		for (int i = 0; i < width; i++)
			if (getVar(i) == BinFunction.INV)
				return false; // One variable is invalid
		for (int i = width; i < cube.length * 32; i++)
			if (getVar(i) != BinFunction.DC)
				return false; // Unused variable > width is not don't care
		return true;
	}
}