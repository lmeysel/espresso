...
public class IntersectFreeSet extends Set {
	...
	public IntersectFreeSet(int width) {
		super(width);
	}

	/**
	 * Adds cube c intersect-free: - Adds c, if c has no intersects with the existing cubes in the
	 * Set. - Don't adds c, if c is completely covered by one existing cube. - Splits up c into
	 * smaller cubes and tries to add them, if c has intersections with the existing cubes.
	 * 
	 * @return Returns true, if something was added
	 */
	@Override
	public boolean add(final Cube c) {
		for (int i = 0; i < this.size(); i++) {
			...
			if (!a.isValid())
				continue; // c has no intersection with cube[i]
			if (a.equals(c))
				return false; // c is completely covered by one existing cube[i]
	...
	public boolean covers(final Cube u) {
		...
		for (int i = 0; i < this.size(); i++) {
			...
			if (ca == -1)
				continue; // no intersection between this[i] and u
			while (ca < cover_cardinality.length)
				if (cover_cardinality[ca]) {
