...
public class Set extends ArrayList<Cube> {
	...
	/**
	 * Checks, weather the given cube u is completely covered by this set. If u is element of this,
	 * the function checks, weather u is covered by this without u
	 * 
	 * @param u
	 * @return
	 */
	...
	private boolean covers(final Cube u, final Cube ignore) {
		...
		for (i = 0; i < this.size(); i++) {
			Cube a = this.get(i);
			if (a == ignore)
				continue; // this without u
			a = u.and(a);
			if (!a.isValid())
				continue; // u has no intersection with cube[i]
			if (a.equals(u))
				return true; // u is completely covered by the existing cube[i]
			for (int j = 0; j < u.width; j++)
				if (u.getVar(j) == BinFunction.DC && a.getVar(j) != BinFunction.DC) {
